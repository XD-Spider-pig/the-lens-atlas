...
function UploadModal({user,onClose,onCreated}){
  ...
  async function submit(e){
    ...
    try{
      ...
      let photo={id:uid(),type:'community',title:title||location.split(',')[0],location_name:location.split(',')[0],city:location,lat:coords.lat,lng:coords.lng,photographer:user?.name||'Ronan',profile_name:user?.name||'Ronan',image_url:preview,camera,lens,conditions:conditions||'Not specified',tags:['Community'],description:description||'Shared by the LensAtlas community.',likes:0,comments:0,privacy};
